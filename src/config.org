...
** Client
#+PROPERTY: header-args :tangle "Client.hs"
...
sendCommand Command{addr, command_, input} = do
  let joinedInput = unwords (command_:input)
  display <- openDisplay ""
  rootWin <- rootWindow display $ defaultScreen display
  addrAtom <- internAtom display addr False
  msgAtom <- internAtom display joinedInput False
  allocaXEvent $ \event -> do
                  setEventType event clientMessage
                  setClientMessageEvent event rootWin addrAtom 32 msgAtom currentTime
                  sendEvent display rootWin False structureNotifyMask event
                  sync display False
#+end_src

Then we write a command line parser to create an instance of this type.
#+begin_src haskell
...
Finally, plumb it all together
#+begin_src haskell
main :: IO ()
main = execParser opts >>= sendCommand
  where
    opts = info (helper <*> commandParser)
      (header "Send a command to the XMonad server"
       <> progDesc "Send ARGS to the xmonad server listener at ADDR"
       <> fullDesc)
#+end_src

This can be run through the shell script =bin/client= for easier access.
Keep in mind that if you're testing the client script through a terminal in your current X session,
then the =DISPLAY= variable needs to be set appropriately.
...
around this is to switch to =Super= (=mod4Mask=) when an additional =--test= argument is passed. This is handled by =handleArgs=.
#+begin_src haskell
myModMask   = mod1Mask
...
We handle arguments here.
For now, there's only two conditions we care about: is this a test environment or not.
In case it is, we want to switch the mod key and log to the terminal.
Otherwise, we use the default mod key and log to files to /tmp.
#+begin_src haskell
handleArgs :: XConfig l -> IO (XConfig l)
handleArgs config = getArgs >>= \case
  []         -> pure $ redirectLogs "/tmp" $ config
  ["--test"] -> pure $ config { modMask = testModMask }
  args       -> hPutStrLn stderr ("Unexpected arguments encountered: " ++ show args)
                >> pure config
#+end_src
