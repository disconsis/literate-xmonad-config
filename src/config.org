...
shrinkWinForDeco direction _deco@(Rectangle _ _ dw dh) _win@(Rectangle x y w h) =  case direction of
  U -> Rectangle x  y' w  h'
  D -> Rectangle x  y  w  h'
  L -> Rectangle x' y  w' h
  R -> Rectangle x  y  w' h
...
instance Eq a => DecorationStyle SmartTabAddonDeco a where
  ...
  pureDecoration (SmartTabAddonDeco direction) decoWidth decoHeight _ _ windowRects _win@(_, Rectangle x y w h)
    | length windowRects >= 2 = Just smartTabBar
    | otherwise               = Nothing
    where
      smartTabBar = case direction of
        U -> Rectangle x  y  w         decoHeight
        D -> Rectangle x  y' w         decoHeight
        L -> Rectangle x  y  decoWidth h
        R -> Rectangle x' y  decoWidth h
      y' = (y + fi (h - decoHeight))
      x' = (x + fi (w - decoWidth))
#+end_src
