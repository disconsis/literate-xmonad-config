...
displayText text = io . void $ readProcess "dzen2" ["-p", "-l", show numLines, "-ta", "c", "-sa", "c", "-e", "onstart=uncollapse"] text
  where
    numLines = max 0 (length (lines text) - 1)          -- we only count slave lines, so everything after the first one
#+end_src

Like emacs' =which-key=, the help message activates after a delay and closes by itself if any key is pressed.
...
work around it using the functions it /does/ export. See [[opening-apps]] for a usage example. We add ~?~ (aka ~S-<slash>~) to display the help before the delay.
...
#+begin_src haskell
whichkeyDelay = seconds 3
whichkeyBind = (shiftMask, xK_slash)

whichkeySubmap :: (LayoutClass l Window, Read (l Window)) => XConfig l -> [(String, NamedAction)] -> X ()
whichkeySubmap config keybinds = do
  pid <- xfork (threadDelay whichkeyDelay >> displayText (toHelp keymap))
  catchX (submap $ keys config' config' { layoutHook = Layout (layoutHook config') }) mempty
  io $ signalProcess sigKILL pid
  spawn "pkill dzen2"
  where
    toHelp = unlines . showKm
    whichkeyAction = addName "Show this help" . io . displayText . toHelp
