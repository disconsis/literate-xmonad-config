...
withCurrentWorkspace action =
  withWindowSet (W.current .> W.workspace .> W.tag .> action)
#+end_src
* Markup(s)
...
We use =X.A.WorkspaceNames= for this.
=workspaceNamesPP= from that module makes a pretty-printer print the workspace names.

We also need a prompt for reading the workspace name with =renameWorkspace=.
...
renameWorkspace' prompt = withCurrentWorkspace $ \ws ->
  when (not (ws `member` myPredefinedWorkspaces))
