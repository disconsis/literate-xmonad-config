...
displayTextSync :: MonadIO m => Maybe Rational -> String -> m ()
...
displayTextSyncTill, displayTextTill :: MonadIO m => Rational -> String -> m ()
displayTextSyncTill = displayTextSync . Just
displayTextTill = displayText . Just

displayTextSyncForever, displayTextForever :: MonadIO m => String -> m ()
...
buildConfig =
  io $ (try (system "bin/build") :: IO (Either SomeException ExitCode)) >>= \case
    ...
    Right (ExitFailure failCode) -> displayTextTill errTime (printf "Build failed with code: %d" failCode) >> return False
    Left exception -> displayTextTill errTime ("Build failed with exception: " <> displayException exception) >> return False
  where
    errTime = 5
